...
print("--------------------------------------")
print("[DB] > fun_player  初期化完了")
print("[DB] > mid_player  初期化完了")
print("[DB] > open_player  初期化完了")
print("--------------------------------------")
...
for i in range(1,145):
    ...
    sql = "INSERT INTO fun_player (id, player, team ) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE player = VALUES(player), team = VALUES(team)"
...
for i in range(1,145):
    temp = (ws.cell(i,1).value, ws.cell(i,2).value, ws.cell(i,3).value)
    sql = "INSERT INTO mid_player (id, player, team ) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE player = VALUES(player), team = VALUES(team)"
    cursor.execute(sql, temp)
    conn.commit()
...
for i in range(1,145):
    temp = (ws.cell(i,1).value, ws.cell(i,2).value, ws.cell(i,3).value)
    sql = "INSERT INTO open_player (id, player, team ) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE player = VALUES(player), team = VALUES(team)"
    cursor.execute(sql, temp)
    conn.commit()
